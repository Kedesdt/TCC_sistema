...
async function atualiza(pacienteId) {
    ...
    switch (gParametro){
        case "Oxigenação":
            await dataTotal.forEach((d) => {
                ...
                if (d.oxi > 10 && d.pacienteId == pacienteId){
        ...
        case "Freq. Cardiaca":
            await dataTotal.forEach((d) => {
                ...
                if (d.freq > 10 && d.pacienteId == pacienteId){
                    //labels.push(converteData(d.dataHora));
                    data.push(d.freq);
        ...
        case "Acelerometro":
            await dataTotal.forEach((d) => {
                if (d.pacienteId == pacienteId){
                    let dd = Math.sqrt(d.ax*d.ax + d.ay*d.ay + d.az*d.az) / 2**14;
                    data.push(dd);
        ...
        case "Temperatura":
            await dataTotal.forEach((d) => {
                ...
                if (d.temp > 10 && d.pacienteId == pacienteId){
                    //labels.push(converteData(d.dataHora));
                    data.push(d.temp)
